...
export default function TweetsPage() {
  ...
  useEffect(() => {
    ...
    // Socket.io listeners
    socket.on("new-tweet", (tweet: Tweet) => {
      setTweets((prev) => [tweet, ...prev]);
    });

    socket.on("delete-tweet", (tweetId: string) => {
      setTweets((prev) => prev.filter((t) => t._id !== tweetId));
    });

    socket.on("edit-tweet", (updated: Tweet) => {
      setTweets((prev) =>
        prev.map((t) => (t._id === updated._id ? updated : t))
      );
    });

    socket.on("like-tweet", ({ tweetId, isLiked, likesCount }) => {
      setTweets((prev) =>
        prev.map((t) => (t._id === tweetId ? { ...t, isLiked, likesCount } : t))
      );
    });

    return () => {
      socket.off("new-tweet");
      socket.off("delete-tweet");
      socket.off("edit-tweet");
      socket.off("like-tweet");
    };
  }, [user]);
  ...
  const handleCreateTweet = async (e: React.FormEvent) => {
    ...
    try {
      ...
      socket.emit("new-tweet", response.data);
      setNewTweet("");
    } catch (err) {
  ...
  const handleEditTweet = async (tweetId: string) => {
    ...
    try {
      ...
      socket.emit("edit-tweet", response.data);
  ...
  const handleLikeTweet = async (tweetId: string) => {
    try {
      ...
      const likesCount = isLiked
        ...
        : (tweet.likesCount || 0) - 1;
      socket.emit("like-tweet", { tweetId, isLiked, likesCount });
    } catch (err) {
          ...
          {loading ? (
                        ...
                        <>
                          ...
                          <div className="flex items-center space-x-6 text-muted-foreground">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-auto p-0 hover:text-blue-500"
                            >
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                Share your thoughts with your first tweet!
              </p>
            </div>
          )}
        </div>
